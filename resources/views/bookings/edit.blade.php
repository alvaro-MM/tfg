...
<div class="container mx-auto p-6 max-w-2xl bg-white shadow-md rounded-lg">
    ...
    <form action="{{ route('bookings.update', $booking) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $booking->name) }}" class="border rounded-lg p-3 w-full focus:ring-2 focus:ring-indigo-500" required>
        </div>
        ...
        <div>
            ...
            <select name="table_id" id="table_id" class="border rounded-lg p-3 w-full focus:ring-2 focus:ring-indigo-500" required>
                ...
                <option
                    value="{{ $table->id }}"
                    @selected(old('table_id', $booking->table_id) == $table->id)
                    >
        ...
        <div>
            ...
            <input type="date" name="booking_date" id="booking_date" value="{{ old('booking_date', $booking->booking_date->format('Y-m-d')) }}" class="border rounded-lg p-3 w-full focus:ring-2 focus:ring-indigo-500" required>
        </div>
        ...
        <div>
            ...
            <select name="booking_time" id="booking_time" class="border rounded-lg p-3 w-full focus:ring-2 focus:ring-indigo-500" required>
                ...
                <optgroup label="Comida (12:00 - 16:00)">
                    @for ($h = 12; $h <= 15; $h++)
                        ...
                        @endphp
                        <option value="{{ $time }}" @selected(old('booking_time', $booking->booking_time) === $time)>
                            {{ substr($time, 0, 5) }}
                        </option>
                        @endfor
                        @endfor
            ...
            <p class="mt-2 text-sm text-gray-500">Duración automática de la reserva: <strong>90 minutos</strong></p>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="border rounded-lg p-3 w-full focus:ring-2 focus:ring-indigo-500">{{ old('description', $booking->description) }}</textarea>
        </div>

        <div class="flex justify-between items-center">
            <a href="{{ route('bookings.index') }}" class="text-gray-600 hover:underline">
                ← Volver
            </a>
