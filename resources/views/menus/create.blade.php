<x-layouts.app :title="__('Crear menú')">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900 dark:text-stone-100">Crear menú</h1>
    ...
    <div class="mt-4">
        ...
        <form action="{{ route('menus.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-stone-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            </div>
            ...
            <div>
                ...
                <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-stone-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    ...
                    <option value="daily" {{ old('type') === 'daily' ? 'selected' : '' }}>Diario</option>
                    <option value="special" {{ old('type') === 'special' ? 'selected' : '' }}>Especial</option>
                    <option value="seasonal" {{ old('type') === 'seasonal' ? 'selected' : '' }}>Estacional</option>
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price') }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-stone-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            </div>
                ...
                <div class="mt-2 space-y-2">
                    @foreach($dishes as $dish)
                        <label class="flex items-center">
                            <input type="checkbox" name="dishes[]" value="{{ $dish->id }}" {{ in_array($dish->id, old('dishes', [])) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="flex justify-end">
                <button type="submit" class="rounded bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">Crear menú</button>
            </div>
        </form>
    </div>
</x-layouts.app>